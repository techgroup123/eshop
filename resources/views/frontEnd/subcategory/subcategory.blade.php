	...
	<div class="container">
			...
			<div id="sidebar" class="col-md-3 filter-sidebar filter-elements">
				<div class="filter-group">
					<div class="filter-item filter_categories ">
						<h4 class="title">All In One {{$subcategory->subcat_name}}</h4>
						<ul class="subcats">

                            <?php
                            $brands=\App\Brand::where('subcat_id',$subcategory->id)->get();
                            ?>
							@if(count($brands)>0)
								@foreach($brands as $brand)
									<li><a href="{{ route('brand.posts',$brand->slug) }}">{{$brand->brand_name}}</a></li>
								@endforeach
							@endif
						</ul>					
					</div>
		...
		<a href="{{url('/')}}" itemprop="item">
			<span itemprop="name">Home</span>
						...
						<a href="{{ route('subcategory.posts',$subcategory->slug) }}" itemprop="item">
							<span itemprop="name">{{$subcategory->subcat_name}}</span>
						</a>
					</li><li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
